...
assert np.isclose(
    all_corrects.mean(), 1 / 10, atol=0.05
), "Random agent is not random enough!"
...
class UCBActionSelection(Agent):
    ...
    def get_action(self):
        A_t = np.argmax(self.Q + self.c * np.sqrt(np.log(self.t)) / self.N)
        self.t += 1
        return A_t
    ...
    def reset(self, seed: int):
        super().reset(seed)
        self.N = np.ones(self.num_arms) * self.eps
        # np.full(...)
        self.Q = np.zeros(self.num_arms)  # should be zeros i think
...
imshow(
    ...
    facet_col=-1,
    facet_labels=[f"s_next = {s}" for s in states],
    y=states,
    x=actions,
...
def policy_eval_numerical(
    ...
    for i in range(max_iterations):
        ...
        for s in range(num_states):
            ...
            #     bellman += env.T[s,pi[s],ss]*(env.R[s,pi[s],ss] + gamma * V_old[ss])
            # V_new[s] = bellman

            V_new[s] = env.T[s, pi[s]] @ (env.R[s, pi[s]] + gamma * V_old)
        if max(abs(V_new - V_old)) < eps:
            break
        else:
            V_old = V_new
    return V_new
...
def policy_eval_exact(env: Environment, pi: Arr, gamma=0.99) -> Arr:
    ...
    for i in range(num_states):
        for j in range(num_states):
            P[i, j] = env.T[i, pi[i], j]
            R[i, j] = env.R[i, pi[j], j]

    # r = np.zeros(num_states)
    # for i in range(num_states):
    #     r[i] = (P @ R.T)[i, i]
    r = einops.einsum(P, R, "s s_next, s s_next -> s")
    return np.linalg.inv(np.eye(num_states) - gamma * P) @ r
...
def policy_improvement(env: Environment, V: Arr, gamma=0.99) -> Arr:
    ...
    """
    a = einops.einsum(env.T, env.R, "s a s_next, s a s_next -> s a")
    b = einops.einsum(env.T, gamma * V, "s a s_next, s_next -> s a")
    c = a + b
    pi = np.argmax(c, axis=-1)
    return pi
