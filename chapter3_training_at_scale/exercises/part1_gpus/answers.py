...
def test_daxpy_random_input(fn1, fn2):
    ...
    assert torch.allclose(
        fn1(alpha, x, y), fn2(alpha, x, y), 0, 1e-6
    ), "Implementations are not analogous"
    print("Tests passed")
...
bench_results = []
for contender in [naive_softmax_jit, naive_softmax]:
    try:
        name = contender.__name__
    except:
        name = contender.name
    ...
    t = benchmark.Timer(
        setup="x = torch.rand(1000,1000, device='cuda')",
        stmt="function(x)",
        description=f"cuda",
        label="daxpy",
        sub_label=name,
...
def quantize_tensor(x, min_val=None, max_val=None, num_bits=8) -> QTensor:
    ...
    tensor = (
        (zero_point + x / scale).round().byte()
    )  # not using round fails for some reason (not clamping is fine)
...
def dequantize_tensor(q_x) -> torch.tensor:
    ...
    """
    x_float = q_x.scale * (q_x.tensor - q_x.zero_point)
    return x_float
...
def quantizeLayer(x, layer, stat, scale_x, zp_x) -> Tuple[torch.tensor, float, float]:
    ...
    """
    # Shift the input tensor using the zp_x argument
    x_q = x / scale_x + zp_x
    # Calculate the scale and zero point of the next layer using the calcScaleZeroPoint function
    scale_w, zp_w = calcScaleZeroPoint(
        min_val=layer.weight.data.min(), max_val=layer.weight.data.max()
    )
    scale_b, zp_b = calcScaleZeroPoint(
        min_val=layer.bias.data.min(), max_val=layer.bias.data.max()
    )
    # Quantize the weights and biases of the layer using the calculated scale and zero point
    orig_weight = layer.weight.copy()
    orig_bias = layer.bias.copy()
    layer.weight.data = quantize_tensor(layer.weight.data)
    layer.bias.data = quantize_tensor(layer.bias.data)

    # Execute a forward pass of the input tensor through the quantized layer, apply a ReLU and obtain the output tensor
    x_output = F.relu(layer(x_q))
    # Finally, we reset the layers weights and biases to their original values
    layer.weight.data = orig_weight
    layer.bias.data = orig_bias
    # Return the output tensor, scale and zero point of the next layer
    return x_output, calcScaleZeroPoint()
...
def add_and_subtract():
    ...
    adder_thread = Thread(target=adder, args=(1, 1_000_000))
    adder_thread.start()
    # Start a thread making subtractions
    # time.sleep(0.05)
    subtractor_thread = Thread(target=subtractor, args=(1, 1_000_000))
