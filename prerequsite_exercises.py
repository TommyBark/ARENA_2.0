...
def rearrange_3() -> t.Tensor:
    ...
    [[[1], [2], [3], [4], [5], [6]]]
    """
    return rearrange(t.arange(1, 7), " (h w i) -> h w i ", h=1, w=6, i=1)
...
def temperatures_average(temps: t.Tensor) -> t.Tensor:
    ...
    return reduce(temps, "(w h) -> w", "mean", h=7)
...
def temperatures_differences(temps: t.Tensor) -> t.Tensor:
    ...
    """
    assert len(temps) % 7 == 0
    return temps - repeat(temperatures_average(temps), "w -> (w n)", n=7)
...
def temperatures_normalized(temps: t.Tensor) -> t.Tensor:
    ...
    """
    std = (
        reduce(temperatures_differences(temps) ** 2, "(w h) -> w", "sum", h=7) / 6
    ) ** (1 / 2)
    # std = reduce(temps, "(w 7) -> w", t.std)
    std = repeat(std, "w -> (w n)", n=7)
...
def batched_dot_product_nd(a: t.Tensor, b: t.Tensor) -> t.Tensor:
    ...
    return einops.einsum(a, b, "batch ...,batch ... -> batch")


a = t.tensor([[1, 1, 0], [0, 0, 1]])
b = t.tensor([[1, 1, 0], [1, 1, 0]])
actual = batched_dot_product_nd(a, b)
...
def classifier_accuracy(scores: t.Tensor, true_classes: t.Tensor) -> t.Tensor:
    ...
    return (t.argmax(scores, dim=-1) == true_classes).sum() / true_classes.shape[0]
...
def batched_logsumexp(matrix: t.Tensor) -> t.Tensor:
    ...
    exp = t.exp(matrix - rearrange(C, "n -> n 1"))
    s = exp.sum(dim=1)
    l = C + t.log(s)
    return l
...
def batched_logsoftmax(matrix: t.Tensor) -> t.Tensor:
    ...
    """
    C = matrix.max(dim=-1).values
    s = batched_softmax(matrix - rearrange(C, "n -> n 1"))
    l = t.log(s)
    # return t.log(C) + l ## TODO: Finish!
    return t.log_softmax(matrix, dim=1)
...
def batched_cross_entropy_loss(logits: t.Tensor, true_labels: t.Tensor) -> t.Tensor:
    ...
    return rearrange(
        -t.gather(logprob, dim=1, index=rearrange(true_labels, "n -> n 1")), "n 1 -> n"
    )
